...
int main(int argc,char **argv) 
{
	...
	if ( (z = socketpair(AF_LOCAL,SOCK_STREAM,0,s)) == -1 ) {
	  fprintf(stderr,"%s: socketpair(2)\n", strerror(errno)); exit(1); }
	printf("Created socket pair with these File Descriptors: %d %d\n", s[0],s[1]);
	...
	chpid = fork();
	printf("chpid: %d pid_t: %d ((pid_t)-1): %d\n", chpid, ((pid_t)+0), ((pid_t) - 1) );
	if ( chpid == (pid_t)-1 ) {
	...
	else if ( chpid == 0 ) 
	{
		...
		// Now indicate that we will not be writing  anything further to our socket, by shutting down the write side of the socket:
	...
	else 
	{
		...
		//Request a read; the parent process waits here for a request from the client:
