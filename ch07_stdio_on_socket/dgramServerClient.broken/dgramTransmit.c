...
{
  char txDgramBuffer[512]="Hello World\n"; //DATA
  ...
  FILE *tx = NULL;
  tx = fdopen(txSocket, "w");
  if ( !tx ){ close(tx); printf("Shit hit the fan\n"); return 0;}
  setbuf(tx, _IOFBF);
  printf("str len: %d\n", strlen(txDgramBuffer) );
  printf("str len: %s\n", txDgramBuffer);
  fwrite(txDgramBuffer, 8, strlen(txDgramBuffer) , tx);
  fflush(tx);
  fclose(tx);
