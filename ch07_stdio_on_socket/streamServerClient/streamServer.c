...
int main(int argc, char **argv)
{
	...
	for (;;)
	{
	  ...
	  // Reply to Client with the message (the 2 commented out line below are replaced with buffered FILE IO):
	  //succ_or_fail = write(conn_sock, buffoon, strlen(buffoon));
	  //if ( succ_or_fail == -1) bail("bailing at write() on line __LINE__");
	  tx = fdopen(conn_sock, "w");
	  if ( !tx ){ close(tx); continue;}
	  setbuf(tx, _IOFBF);
	  fwrite(buffoon, 8, strlen(buffoon) + 1 , tx);
	  fflush(tx);
	  fclose(tx);
