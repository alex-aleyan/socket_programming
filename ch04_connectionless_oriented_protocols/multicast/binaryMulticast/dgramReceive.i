# 1 "dgramReceive.c"
# 1 "/home/e313837/Tutorials/socket_programming/ch04_connectionless_oriented_protocols/multicast/binaryMulticast//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "dgramReceive.c"
...
int main(int argc, char *argv[])
{
  ...
  localRxSock = socket(2, SOCK_DGRAM, 0);
  ...
  int reuse = 1;
  if(setsockopt(localRxSock, 1, 2, (char *)&reuse, sizeof(reuse)) < 0) {
 perror("Setting SO_REUSEADDR error"); close(localRxSock); exit(1); }
  ...
  localSock.sin_family = 2;
  localSock.sin_port = htons(4321);
  localSock.sin_addr.s_addr = ((in_addr_t) 0x00000000);
 ...
 perror("Binding datagram socket error"); close(localRxSock); exit(1); }
  ...
  mcastGroup.imr_multiaddr.s_addr = inet_addr("226.1.1.1");
  mcastGroup.imr_interface.s_addr = inet_addr("192.168.40.185");
  if (argc >= 2) mcastGroup.imr_interface.s_addr = inet_addr(argv[1]);

  if(setsockopt(localRxSock, IPPROTO_IP, 35, (char *)&mcastGroup, sizeof(mcastGroup)) < 0) {
