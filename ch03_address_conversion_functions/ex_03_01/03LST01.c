...
int main()//int argc,char **argv) 
{
	...
	addresses[] =		{
																{ { 44,135,86,12 } },							//address[0]
																{ { 127,0,0,1 } },										//address[1]
																{ { 172,16,23,95 } },							//address[2]
																{ { 192,168,9,1 } }					};		//address[3]

 // printf("sizeof addresses[]: %d \n",(sizeof(addresses)>>2)); // divide by 4 since each address occupies 4 bytes
	for (		x=0;				x<(sizeof(addresses)>>2);			++x ) // for x starting at 0; for x less than the number of addresses in the addresses[]; increment
	{
	/*
	...
	memcpy(&adr_inet.sin_addr.s_addr, addresses[x].ip,4); // for x=0 address[0] is copied, for x=1 address[1] is copied...
